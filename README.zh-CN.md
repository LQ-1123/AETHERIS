...
v0.3.0 将患者队列、阅片、检查申请、结构化报告、独立审核、账号管理和工作量统计串成了一套机构级工作流。
...
完整功能、升级步骤、验证结果和已知限制见 [v0.3.0 发布说明](doc/releases/v0.3.0.md)。

---

## 界面截图

| 登录                               | 主界面（病人列表 + 阅片）                    | 多窗口阅片                             |
| ---------------------------------- | -------------------------------------------- | -------------------------------------- |
| ![登录](doc/screenshots/login.png) | ![主界面](doc/img/更新的主界面.png)          | ![多窗口阅片](doc/img/多窗口图像.png)  |

| 多角度 MPR 重建                    | 体渲染                                          | AI 分割                                         |
| ---------------------------------- | ----------------------------------------------- | ----------------------------------------------- |
| ![MPR 重建](doc/img/多角度MPR重建.png) | ![体渲染](doc/screenshots/volume-rendering.png) | ![AI 分割](doc/screenshots/ai-segmentation.png) |

| 报告工作台（医生）                        | 标注                                     | DICOM TAG 修订                                |
| ----------------------------------------- | ---------------------------------------- | --------------------------------------------- |
| ![报告工作台](doc/img/报告书写界面-医生.png) | ![标注](doc/screenshots/annotations.png) | ![TAG 修订](doc/screenshots/tag-revision.png) |

| 管理控制台                        | 生命周期                                   | DICOM 路由引擎                          |
| --------------------------------- | ------------------------------------------ | --------------------------------------- |
| ![管理控制台](doc/img/管理控制台.png) | ![生命周期](doc/screenshots/lifecycle.png) | ![路由引擎](doc/screenshots/router.png) |
...
登录后默认进入全屏患者检查队列，一行对应一次检查。队列支持服务端分页、排序，以及来源机构、报告状态、模态、部位和日期组合筛选。

技师和管理员可以按两种顺序处理申请单：

* 先创建申请单，接收影像后再手动匹配检查。
* 直接从已经入库的检查开具申请单；患者和 Study 数据由服务端读取，不信任客户端自行填写的关联信息。

申请单携带模态、部位、申请类型和临床指征，按“待执行 → 已执行 → 已完成”流转。报告工作台会展示关联申请单，报告审核通过或签发后申请单自动完成。

## 独立报告工作台

诊断报告在独立桌面窗口中完成，Viewer 保持专注于影像显示与操作。报告按检查管理，支持：

* 结构化所见、印象、建议和阳性标记
* 保存草稿与不可变的签发版本快照
* 提交审核、领取审核、审核人修正、审核签发和签发后修订
* 患者/机构信息、申请单临床指征、审核人身份和完整审核时间线

## 机构级独立审核

管理员可在“管理控制台 → 机构设置”开启或关闭报告审核闭环。设置按机构持久化并即时生效。

开启后，作者只能保存草稿和提交审核，不能直接签发；审核人必须拥有 `review_report` 权限，并且不能审核自己的报告。审核人修正报告时保留原作者署名、记录审核人，并写入 `reviewer_modified` 审计事件，供工作量和错误统计使用。关闭时保留草稿直签路径，适用于单医生或演示环境。

## 管理控制台与工作量

管理控制台提供账号创建、首次登录强制改密、停用/启用与会话吊销、审核权限授予、设备注册、来源归属、用户授权、机构设置和按人员统计的工作量报表。

密码重置采用审批制：用户在登录页提交用户名和拟使用的新密码，系统只保存 Argon2id 哈希；管理员可以批准或驳回，但看不到密码内容。工作量报表按日期范围聚合报告状态、签发版本、完成审核、审核修正和申请单数量。

账号、报告、设备、申请单、工作量和机构设置的权限及机构边界均由服务端强制执行，而不是只依靠前端隐藏入口。
...
* [Windows 10/11 x64 安装器](https://github.com/LQ-1123/AETHERIS/releases/download/v0.3.0/AETHERIS-Setup-0.3.0-x64.exe)

<p align="center"><img src="doc/diagrams/packaging.svg" alt="双平台打包结构" width="620"/></p>

macOS dmg 开箱即用；Windows 安装包目标机无需单独安装 PACS。

v0.3.0 安装包尚未配置商业代码签名：macOS 使用 ad-hoc 签名且未 notarization，Windows 安装器需要管理员权限。安装或升级前请阅读 [发布说明](doc/releases/v0.3.0.md)。
...
* [ ] 更多 AI 分割模型
* [ ] AI 辅助影像分析
* [x] 结构化报告（报告工作台：所见/印象/建议、阳性标记、签发/修订、版本快照）
* [x] 报告审核工作流（独立审核、审核人直接修正、机构级开关）
* [x] 检查申请单工作流（先申请后上传、已入库检查直接开单、手动匹配）
* [x] 工作量报表（按人员与日期范围统计）
* [ ] DICOM SR 集成
* [x] 高级工作列表管理
* [ ] 分布式存储
